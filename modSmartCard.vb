...
Friend Module modSmartCard
    ...
    Private Structure SCARD_IO_REQUEST
        Public dwProtocol As Integer ' identifies the protocol in use
        Public dbPciLength As Integer ' length of the structure
    End Structure

    ' Chiave nel registro di Window sper la registrazione dell'ATR
    Private Const REG_ATR As String = "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Cryptography\Calais\SmartCards\"
    ...
    Friend Function SCardIO(ByVal hCard As Long, ByRef aSendData() As Byte, ByRef aRecvData() As Byte, ByRef pbSW1 As Byte, ByRef pbSW2 As Byte) As Long
        Dim ret As Integer
    ...
    Friend Function SCardRead(ByVal hCard As Integer, ByVal pbCLA As Byte, ByVal pbINS As Byte, ByVal pbP1 As Byte, ByVal pbP2 As Byte, ByVal pbLen As Byte, ByRef lpszRecvData As String, ByRef pbSW1 As Byte, ByRef pbSW2 As Byte) As Integer
        ...
        pbSW1 = 0 ' inizializza il parametro SW
        pbSW2 = 0 ' inizializza il parametro SW
        lpszRecvData = vbNullString ' inizializza la stringa che conterrà i dati ricevuti
        ' imposta il protocollo
        xIO.dwProtocol = SCARD_PROTOCOL_T0
        xIO.dbPciLength = Len(xIO) ' imposta lunghezza struttura
    ...
    Friend Function ReaderStatus(ByRef pdwState As Integer, ByRef pdwProtocol As Integer, ByRef pszATR As String, Optional ByRef pszReaderName As String = vbNullString) As Integer
        ...
        If ret <> SCARD_S_SUCCESS Then GoTo FreeMemory_ReaderStatus ' se è avventuo un errore, interrompe
        If Len(pszReaderName) = 0 Then ' se non è stato specificato il nome del lettore
            ret = SCardListReaders(lContext, vbNullString, sReaderName) ' recupera il nome del primo lettore disponibilie
            If ret <> SCARD_S_SUCCESS Then GoTo FreeMemory_ReaderStatus ' se è avventuo un errore, interrompe
        Else ' se invece è stato specificato un lettore
...
FreeMemory_ReaderStatus:
    ...
    Friend Function ReaderList() As String()
        ...
        For i = 0 To UBound(aRdrLst) - 1        ' cicla l'array
            ...
            If aRdrLst(i).Length > 0 Then   ' se c'è un lettore
    ...
    Friend Sub SetATR(ByVal sKey As String, ByVal sATR As String, ByVal sMask As String)
        Dim aATR() As Byte = Array.CreateInstance(GetType(Byte), 0)
        Dim aMask() As Byte = Array.CreateInstance(GetType(Byte), 0)
        Dim aTmp() As Byte = Array.CreateInstance(GetType(Byte), 0)
        Dim sReg As String
        Dim reg As New Registry

        sReg = REG_ATR & sKey
        If Not reg.ReadByte(sReg, "ATR", aTmp) Then
            Hex2Byte(aATR, sATR)
            Hex2Byte(aMask, sMask)

            ' crea l'ATR nel registro di Windows
            reg.Create(sReg)
            reg.WriteByte(sReg, "ATR", aATR)
            reg.WriteByte(sReg, "ATRMask", aMask)
        End If
    End Sub
    ...
    Private Function Hex2Str(ByRef sHex As String) As String
        Dim i As Integer
        Dim s As String
        s = vbNullString
